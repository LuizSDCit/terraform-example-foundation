...
locals {
  ...
  vpc_name        = "${var.environment_code}-shared-${type}${local.mode}"
  network_name    = "vpc-${local.vpc_name}"
  restricted      = var.type == "restricted"
  googleapis_cidr = local.restricted ? "199.36.153.4/30" : "199.36.153.8/30"
  api_domain      = local.restricted ? var.type : "private"
...
data "google_projects" "net_hub" {
  ...
  filter = "parent.id:${split("/", data.google_active_folder.common.name)[1]} labels.application_name=org-${type}-net-hub lifecycleState=ACTIVE"
}

data "google_compute_network" "vpc_net_hub" {
  count   = var.mode == "spoke" ? 1 : 0
  name    = "vpc-c-shared-${type}-hub"
...
module "main" {
  ...
  routes = concat(
    [{
      name              = "rt-${local.vpc_name}-1000-all-default-${local.api_domain}-api"
      description       = "Route through IGW to allow ${local.api_domain} google api access."
      destination_range = local.googleapis_cidr
